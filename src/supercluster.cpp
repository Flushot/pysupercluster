...
{
    childIds.insert(_childIds.begin(), _childIds.end());
}
    ...
    , extent(_extent)
{
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        clusters.push_back(new Cluster(points[i], 1, childIds, i, -1));
    }
...
std::vector<Cluster*> SuperCluster::cluster(const std::vector<Cluster*> &points, int zoom)
{
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        if (foundNeighbors) {
            ...
            Cluster *cluster = new Cluster(Point(wx / numPoints, wy / numPoints), numPoints, childIds, clusterId, zoom + 1);
